...
export const Messages: React.FC<MessagesProps> = ({
    ...
    const fetchConversations = async () => {
      try {
        ...
        if (safeData.length > 0 && safeData[0]) {
          ...
          setMessages(
            Array.isArray(safeData[0].messages) ? safeData[0].messages : []
          );
        } else {
  ...
  const handleSelectConversation = async (conversation: Conversation) => {
    try {
      ...
      // Update conversation list to reflect read status - DEFENSIVE
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update messages list - DEFENSIVE
      setMessages((prev) =>
        Array.isArray(prev) ? [...prev, sentMessage] : [sentMessage]
      );
      setNewMessage("");

      // Update conversations list with new last message - DEFENSIVE
      setConversations((prev) =>
        Array.isArray(prev)
          ? prev.map((c) =>
            ...
            <>
              <div className="chat-header">
                <div className="chat-header-info">
                  ...
                  />
                  <h3>{selectedConversation.participantName || "Unknown"}</h3>
                </div>
              </div>
