 ...
 * DEFENSIVE: All array operations protected against undefined/null
 */
...
export const Noticeboard: React.FC<NoticeboardProps> = ({ onSelectPost }) => {
  ...
  // Derived safe counts - DEFENSIVE
  const postsList = Array.isArray(posts) ? posts : [];
  const filteredList = Array.isArray(filteredPosts) ? filteredPosts : [];
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        ...
        // DEFENSIVE: Validate response is array
        const safeData = Array.isArray(data) ? data : [];
        setPosts(safeData);
        setFilteredPosts(safeData);
        const cats = noticeboardApi.getCategories();
  ...
  // Handle category filter - DEFENSIVE
  const handleCategoryChange = async (
    ...
    if (category) {
      try {
        ...
        setFilteredPosts(Array.isArray(filtered) ? filtered : []);
        setSearchQuery("");
      } catch (err) {
  ...
  const handleSearch = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (query.trim()) {
      try {
        ...
        setFilteredPosts(Array.isArray(results) ? results : []);
        setSelectedCategory("");
      } catch (err) {
      ...
      if (selectedCategory) {
        try {
          ...
          setFilteredPosts(Array.isArray(categoryPosts) ? categoryPosts : []);
        } catch (err) {
          console.error("Error reloading category posts:", err);
          setFilteredPosts([]);
        }
      } else {
        setFilteredPosts(Array.isArray(posts) ? posts : []);
  ...
  const handleUpvote = async (postId: number, e: React.MouseEvent) => {
    ...
    try {
      ...
      setFilteredPosts(Array.isArray(updatedPosts) ? updatedPosts : []);
    } catch (err) {
  ...
  return (
    <div className="noticeboard-container">
      <h1>Noticeboard</h1>
      <p className="subtitle">{postsList.length}+ announcements and posts</p>
      ...
      <div className="results-info">
        Showing {filteredList.length} of {postsList.length} posts
      </div>
              ...
              >
                <div className="post-header">
                  ...
                  <div className="post-meta">
                    <p className="post-author">{post.authorName || "Anonymous"}</p>
